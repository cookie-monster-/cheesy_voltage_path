...
class Waypoint {
	...
	draw() {
		this.position.draw((this.angle > 0) ? "rgba(120,120,120,0.8)" : null);
	}
...
function update() {
	...
	$('tbody').children('tr').each(function () {
        ...
        waypoints.push(new Waypoint(new Translation2d(x,y), angle, comment));
    });
    drawPoints();
    drawRobot();
}addPoint
...
function drawRobot() {
	//if(waypoints.length > 1) {
		...
		//drawRotatedRect(waypoints[0].position, robotHeight, robotWidth, waypoints[0].angle, true, getColorForSpeed(60), false);

		//var deltaEnd = Translation2d.diff(waypoints[waypoints.length-2].position, waypoints[waypoints.length-1].position);
		//drawRotatedRect(waypoints[waypoints.length-1].position, robotHeight, robotWidth, waypoints[waypoints.length-1].angle, true, getColorForSpeed(60), false);
	//}
	for(var i=0; i<waypoints.length; i++){
		drawRotatedRect(waypoints[i].position, robotHeight, robotWidth, waypoints[i].angle, true, getColorForSpeed(60), false);
...
function importData() {
	...
	$('#upl').change(() => {
		...
		fr.onload = function(e) {
			...
			$("tbody").empty();
			jd.forEach((wpd) => {
				let wp = new Waypoint(new Translation2d(wpd.position.x, wpd.position.y), wpd.angle, wpd.comment);
				// console.log(wp);
				$("tbody").append("<tr>"
...
function showData() {
	...
	//$(".modal > pre").text("bananas taste gud");
	showModal();
}
